...
cache_prefix="cache/supervised_features/dataset2_full100_train_seed60_20260723"
...
log_file="logs/dataset2_full100_train_seed60_20260723.log"
exit_file="logs/dataset2_full100_train_seed60_20260723.exit"
...
uv run --no-sync python scripts/build_dataset2_full100_train_cache.py \
  ...
  --replay-report "$output_dir/replay-report.json" \
  --output-prefix "$cache_prefix" \
...
if [ "$status" -eq 0 ]; then
  uv run --no-sync python scripts/evaluate_dataset2_full100_train.py \
    --checkpoint "$checkpoint" \
    --source-cache-prefix "$source_prefix" \
    --full100-prefix "$cache_prefix" \
