...
scan_tag="dataset2_setwise_high_weight_scan_seed60_20260725"
candidate_tag="d1_champion_d2_setwise_highw_seed60_20260725"
scan_dir="$root/result/$scan_tag"
candidate_dir="$root/result/$candidate_tag"
checkpoint="$root/checkpoints/$candidate_tag.pkl"
log_path="$root/logs/$candidate_tag.log"
pid_path="$scan_dir/pipeline.pid"
exit_path="$scan_dir/pipeline.exit"
...
setsid -f bash -lc "
  ...
  (
    set -e
    uv run --no-sync python scripts/scan_dataset2_setwise_high_weights.py \
      ...
      --output-dir result/$scan_tag/artifacts \
      --batch-size 256 \
    ...
    uv run --no-sync python scripts/build_dataset2_matched_reranker_candidate.py \
      ...
      --evaluation-report result/$scan_tag/artifacts/setwise-high-weight-report.json \
