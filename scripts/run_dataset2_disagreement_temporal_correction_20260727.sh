...
.venv/bin/python scripts/train_dataset2_disagreement_temporal_correction.py \
  --phase all \
  --train-cache-prefix \
  ...
  --oof-expert-logits \
    result/dataset2_pure_jittor_oof_stacking_20260726/oof-expert-logits.npy \
  ...
  --output-dir \
    result/dataset2_disagreement_temporal_correction_20260727 \
  --device cuda
