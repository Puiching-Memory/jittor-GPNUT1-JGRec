...
class TwoTower:
    ...
    def fit(
        ...
        # 预计算一次负采样与全部批特征（负样本由固定 negative_seeds 决定，
        # 与逐 step 重采样结果完全一致），训练循环只做 GPU 前向/反向，
        # 避免每个 step 重复昂贵的逐事件结构负采样导致 GPU 饥饿。
        precomputed = _precompute_tower_batches(sampled_events, negative_seeds, training_context, self.config)
        ...
        for epoch in track(epochs, description="two-tower", total=self.config.epochs, enabled=verbose):
            ...
            for start in range(0, train_size, self.config.batch_size):
                ...
                loss = self._two_tower_loss_from_precomputed(precomputed, batch_idx)
                optimizer.step(loss)
            ...
            val_loss = self._two_tower_val_loss_precomputed(precomputed, val_idx) if val_size > 0 else 0.0
        ...
        del train_idx, val_idx, val_perm, precomputed
        if owns_index:
    ...
    def _two_tower_loss_from_precomputed(self, precomputed: _PrecomputedTowerBatches, batch_idx: np.ndarray) -> jt.Var:
        logits = self.model(
            jt.array(precomputed.src_ids[batch_idx], dtype=jt.int32),
            jt.array(precomputed.src_activity_buckets[batch_idx], dtype=jt.int32),
            jt.array(precomputed.src_recency_buckets[batch_idx], dtype=jt.int32),
            jt.array(precomputed.src_time_buckets[batch_idx], dtype=jt.int32),
            jt.array(precomputed.dst_ids[batch_idx], dtype=jt.int32),
            jt.array(precomputed.dst_popularity_buckets[batch_idx], dtype=jt.int32),
            jt.array(precomputed.dst_recency_buckets[batch_idx], dtype=jt.int32),
            jt.array(precomputed.dst_time_buckets[batch_idx], dtype=jt.int32),
        )
    ...
    def _two_tower_val_loss_precomputed(self, precomputed: _PrecomputedTowerBatches, val_idx: np.ndarray) -> float:
...
@dataclass(frozen=True)
class _PrecomputedTowerBatches:
    src_ids: np.ndarray
    src_activity_buckets: np.ndarray
    src_recency_buckets: np.ndarray
    src_time_buckets: np.ndarray
...
def _precompute_tower_batches(
    ...
    src_ids = np.empty(n, dtype=np.int32)
    src_activity = np.empty(n, dtype=np.int32)
    src_recency = np.empty(n, dtype=np.int32)
    src_time = np.empty(n, dtype=np.int32)
    dst_ids = np.empty((n, candidate_count), dtype=np.int32)
    dst_popularity = np.empty((n, candidate_count), dtype=np.int32)
    dst_recency = np.empty((n, candidate_count), dtype=np.int32)
    dst_time = np.empty((n, candidate_count), dtype=np.int32)
    ...
    return _PrecomputedTowerBatches(
        src_ids=src_ids,
        src_activity_buckets=src_activity,
        src_recency_buckets=src_recency,
        src_time_buckets=src_time,
