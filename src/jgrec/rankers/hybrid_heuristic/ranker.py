...
from .heuristic import HeuristicTower
...
class _DisabledHeuristicTower:
    ...
    def features_for_queries(self, queries: TestQueryArray | list[TestQuery]) -> np.ndarray:
        return _zero_scores(queries, len(HEURISTIC_FEATURE_NAMES))

    def features_for_query_array(self, queries: TestQueryArray) -> np.ndarray:
        return _zero_scores_for_array(queries, len(HEURISTIC_FEATURE_NAMES))
...
class HybridFeatureEncoder:
    def __init__(
        ...
        heuristic_config: HeuristicTowerConfig | None = None,
    ) -> None:
        self.id_map = id_map
        ...
        self.heuristic = _build_heuristic_tower(heuristic_config or HeuristicTowerConfig(enabled=False))
    ...
    def snapshot(self) -> dict[str, Any]:
        return {
            ...
            "heuristic": self.heuristic.snapshot() if hasattr(self.heuristic, "snapshot") else {},
    ...
    def fit(
        ...
        if deterministic_snapshot is not None:
            ...
            elapsed["heuristic"] = perf_counter() - start
            cache_status = "build"
    ...
    def features_for_query_array(self, queries: TestQueryArray) -> np.ndarray:
        ...
        if detail_profile:
            ...
            log_event(f"[feature-profile] first_batch rows={rows} heuristic_start", enabled=True)
        start = perf_counter()
        heuristic_features = _features_from_tower(self.heuristic, queries)
        elapsed = perf_counter() - start
        self._profile_elapsed["heuristic"] += elapsed
        if detail_profile:
            log_event(f"[feature-profile] first_batch rows={rows} heuristic_done elapsed={elapsed:.1f}s", enabled=True)
    ...
    def clear_batch_caches(self) -> None:
        ...
        if hasattr(self.heuristic, "_cn_cache"):
            self.heuristic._cn_cache.clear()
            self.heuristic._cn_decay_cache.clear()
...
def _build_heuristic_tower(config: HeuristicTowerConfig) -> Any:
    if not config.enabled:
        return _DisabledHeuristicTower()
    return HeuristicTower(config)
...
def _feature_masks(feature_count: int, config: TrainingConfig | None = None) -> list[tuple[str, tuple[int, ...]]]:
    ...
    heuristic_end = structure_end + len(HEURISTIC_FEATURE_NAMES)
    profile_end = heuristic_end + len(SOURCE_PROFILE_FEATURE_NAMES)
    ...
    ranges = {
        ...
        "heuristic": _feature_range(structure_end, heuristic_end, feature_count),
        "profile": _feature_range(heuristic_end, profile_end, feature_count),
    ...
    enabled = {
        ...
        "heuristic": config is None or config.heuristic_enabled,
    ...
    for groups in (
        ...
        ("prior", "heuristic"),
        ("prior", "structure"),
        ("prior", "structure", "heuristic"),
        ("prior", "structure", "heuristic", "tower"),
        ("prior", "structure", "heuristic", "tower", "gnn"),
        ("prior", "structure", "heuristic", "tower", "gnn", "seq"),
        ("prior", "target"),
        ("prior", "target", "heuristic"),
        ("prior", "target", "structure"),
        ("prior", "target", "structure", "heuristic"),
        ("prior", "target", "structure", "heuristic", "profile"),
        ("prior", "target", "structure", "heuristic", "profile", "tower"),
        ("prior", "target", "structure", "heuristic", "profile", "tower", "gnn"),
        ("prior", "target", "structure", "heuristic", "profile", "tower", "gnn", "seq"),
    ):
...
def _config_for_selected_features(config: TrainingConfig, feature_indices: tuple[int, ...]) -> TrainingConfig:
    ...
    needs_heuristic = any(structure_end <= idx < heuristic_end for idx in feature_indices)
    needs_profile = any(heuristic_end <= idx < profile_end for idx in feature_indices)
    ...
    selected = replace(
        ...
        heuristic_enabled=config.heuristic_enabled and needs_heuristic,
